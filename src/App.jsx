...
const CAUTS_STORE   = "nw-cautions-v1";
...
/* OrgChart — proper tree diagram with bezier connectors */
function OrgChart({members,contacts,onNodePress}) {
  ...
  function calcSW(id){
    const ch=childOf[id]||[];
    if(ch.length===0){sw[id]=NW;return NW;}
    const total=ch.reduce((s,c)=>s+calcSW(c),0)+HG*(ch.length-1);
    sw[id]=Math.max(NW,total);return sw[id];
  }
  const roots=childOf["__root__"]||[];
  roots.forEach(r=>calcSW(r));
  const pos={};
  function calcPos(id,cx,y){
    pos[id]={cx,y};
    const ch=childOf[id]||[];if(ch.length===0)return;
    const totalW=ch.reduce((s,c)=>s+sw[c],0)+HG*(ch.length-1);
    let x=cx-totalW/2;
    ch.forEach(c=>{calcPos(c,x+sw[c]/2,y+NH+VG);x+=sw[c]+HG;});
  }
  ...
  const svgW=Math.max(totalW,280)+PAD*2;
  const svgH=maxY+NH+PAD;
  return (
        ...
        {members.map(m=>{
          ...
          return (
            <div key={m.id} onClick={()=>contact&&onNodePress(contact)}
              style={{position:"absolute",left:p.cx-NW/2+PAD,top:p.y,width:NW,height:NH,
                background:C.white,borderRadius:12,boxShadow:"0 2px 8px rgba(0,0,0,0.08)",
                border:`1.5px solid ${C.border}`,cursor:contact?"pointer":"default",
...
export default function App() {
  ...
  useEffect(()=>{
    ...
    try{const d=localStorage.getItem(CAUTS_STORE);if(d)setCautions(JSON.parse(d));}catch(_){}
    setReady(true);
  },[]);
  ...
  const saveCauts  = (l)=>{setCautions(l);try{localStorage.setItem(CAUTS_STORE,JSON.stringify(l));}catch(_){}};
  ...
  const exportData=()=>{
    ...
    const url=URL.createObjectURL(blob);const a=document.createElement("a");a.href=url;a.download=`my-network-backup-${new Date().toISOString().slice(0,10)}.json`;a.click();URL.revokeObjectURL(url);
  };
  const importData=(e)=>{
    ...
    reader.onload=(ev)=>{
      try{
        ...
        if(data.cautions){setCautions(data.cautions);localStorage.setItem(CAUTS_STORE,JSON.stringify(data.cautions));}
  ...
  const addContact=()=>{if(!fv("nickname").trim())return;const c=formToContact(form);const updated=[c,...contacts];setContacts(updated);persist(updated);if(form.photo){try{localStorage.setItem(PHOTO_PREFIX+c.id,form.photo);}catch(_){}}setForm(blank(contexts[0]?.id));setShowQInd(false);setShowQCtx(false);setView("list");};
  const saveEdit=()=>{if(!fv("nickname").trim())return;const c=formToContact(form,selected.id);const updated=contacts.map(x=>x.id===selected.id?c:x);setContacts(updated);persist(updated);if(form.photo){try{localStorage.setItem(PHOTO_PREFIX+selected.id,form.photo);}catch(_){}}else if(form.photoCleared){try{localStorage.removeItem(PHOTO_PREFIX+selected.id);}catch(_){}}setSelected(updated.find(x=>x.id===selected.id));setSelectedPhoto(form.photoCleared?null:(form.photo||selectedPhoto||null));setView("detail");};
  const removeContact=()=>{const updated=contacts.filter(c=>c.id!==selected.id);setContacts(updated);persist(updated);try{localStorage.removeItem(PHOTO_PREFIX+selected.id);}catch(_){}setDelConfirm(false);setView("list");};
  ...
  const updateTMember=(mid,field,val)=>setEditTree(p=>({...p,members:p.members.map(m=>m.id===mid?{...m,[field]:val}:m)}));
        ...
        {show&&(
          <div style={{display:"flex",gap:6,marginTop:8}}>
            <input value={val} onChange={e=>setVal(e.target.value)} placeholder={placeholder} autoFocus
              onKeyDown={e=>{if(e.key==="Enter")onAdd();if(e.key==="Escape"){setShow(false);setVal("");}}}
              style={{flex:1,border:`1px solid ${C.border}`,borderRadius:8,padding:"6px 10px",fontSize:13,outline:"none",color:C.text,fontFamily:"inherit"}}/>
            <button onClick={onAdd} style={{background:C.amber,border:"none",borderRadius:8,padding:"6px 10px",color:"#fff",fontSize:12,fontWeight:700,cursor:"pointer",fontFamily:"inherit"}}>Add</button>
          </div>
        )}
      </>
    );
    return (
      <div style={{...WRAP,paddingBottom:30}}>
        ...
        <div style={{padding:"12px 13px 20px",display:"flex",flexDirection:"column",gap:9}}>
          ...
          <InfoCard>
            <FL>Fun Fact / Notes</FL>
            <textarea value={fv("funFact")} onChange={sf("funFact")} rows={3} placeholder="e.g. Speaks 4 languages, ex-F1 engineer, obsessed with ramen…" style={{...INP,resize:"none",lineHeight:1.55}}/>
          </InfoCard>
          <InfoCard>
            <FL>Insecurity / Weakness</FL>
            <textarea value={fv("notes")} onChange={sf("notes")} rows={3} placeholder="Personal vulnerabilities or sensitivities to keep in mind…" style={{...INP,resize:"none",lineHeight:1.55}}/>
          </InfoCard>
          <InfoCard>
            <FL>Social Media</FL>
            {SOCIALS.map((p,i)=>(
        ...
        <div style={{padding:"12px 13px 20px",display:"flex",flexDirection:"column",gap:9}}>
          ...
          {et.members.length>0&&(
            <div>
              ...
              {et.members.map(m=>{const contact=contacts.find(c=>c.id===m.contactId);const dn=contact?displayName(contact):"Unknown";return(
                <InfoCard key={m.id} style={{marginBottom:8}}>
                  <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:10}}>
                    ...
                    <div style={{flex:1}}><div style={{fontSize:14,fontWeight:700,color:C.text}}>{dn}</div>{contact?.role&&<div style={{fontSize:11,color:C.muted}}>{contact.role}</div>}</div>
                    <button onClick={()=>removeTMember(m.id)} style={{background:C.redBg,border:"none",color:C.red,borderRadius:8,width:28,height:28,cursor:"pointer",fontSize:13,display:"flex",alignItems:"center",justifyContent:"center",fontFamily:"inherit"}}>✕</button>
                  </div>
                  <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:8}}>
                    ...
                    <input value={m.role} onChange={e=>updateTMember(m.id,"role",e.target.value)} placeholder="e.g. CEO, Father, Captain…" style={{flex:1,border:`1px solid ${C.border}`,borderRadius:8,padding:"6px 10px",fontSize:13,outline:"none",color:C.text,fontFamily:"inherit",background:"transparent"}}/>
                  </div>
                  <div style={{display:"flex",alignItems:"center",gap:8}}>
                    ...
                    <select value={m.parentMemberId||""} onChange={e=>updateTMember(m.id,"parentMemberId",e.target.value||null)} style={{flex:1,border:`1px solid ${C.border}`,borderRadius:8,padding:"6px 10px",fontSize:13,color:C.text,background:C.white,fontFamily:"inherit",outline:"none"}}>
                      <option value="">— Root (no parent) —</option>
                      {et.members.filter(x=>x.id!==m.id).map(x=>{const xc=contacts.find(c=>c.id===x.contactId);const xn=xc?displayName(xc):"?";return <option key={x.id} value={x.id}>{xn}{x.role?` (${x.role})`:""}</option>;})}
                    </select>
                  </div>
                </InfoCard>
              );})}
          ...
          <InfoCard>
            ...
            <div style={{maxHeight:220,overflowY:"auto",display:"flex",flexDirection:"column",gap:6}}>
              {contacts.length===0?<div style={{fontSize:13,color:C.muted,textAlign:"center",padding:"12px 0"}}>No contacts yet.</div>:
              available.map(c=>{const a=added.has(c.id);return(
                <div key={c.id} onClick={()=>!a&&addTMember(c.id)} style={{display:"flex",alignItems:"center",gap:10,padding:"9px 10px",borderRadius:10,background:a?"#F9FAFB":C.white,cursor:a?"default":"pointer",border:`1px solid ${C.border}`,opacity:a?0.5:1}}>
                  ...
                  <span style={{fontSize:16,color:a?C.muted:C.amber,fontWeight:700,flexShrink:0}}>{a?"✓":"+"}</span>
                </div>
              );})}
            </div>
          </InfoCard>
          ...
          {et.id&&(!treeDelCfm?<button onClick={()=>setTreeDelCfm(true)} style={{background:"transparent",border:`1.5px solid ${C.red}`,borderRadius:14,padding:13,color:C.red,fontSize:14,fontWeight:700,cursor:"pointer",fontFamily:"inherit"}}>Delete This Tree</button>:(
            <InfoCard style={{border:`1px solid ${C.redBg}`}}>
              <div style={{fontSize:14,fontWeight:600,color:C.text,marginBottom:12,textAlign:"center"}}>Delete "{et.name}"?</div>
              <div style={{display:"flex",gap:9}}>
                <button onClick={()=>setTreeDelCfm(false)} style={{flex:1,padding:11,border:`1px solid ${C.border}`,background:"transparent",borderRadius:11,fontSize:14,fontWeight:600,color:C.muted,cursor:"pointer",fontFamily:"inherit"}}>Cancel</button>
        ...
        <div style={{padding:"12px 13px 80px"}}>
          {selTree.members.length===0?(
            <div style={{textAlign:"center",padding:"52px 24px",color:C.muted}}>
              ...
              <div style={{fontSize:13,lineHeight:1.6}}>Tap Edit to add people and define the hierarchy.</div>
            </div>
          ):<OrgChart members={selTree.members} contacts={contacts} onNodePress={c=>openDetail(c,"tree")}/>}
        </div>
      </div>
    );
  }
  ...
  if(cautView==="editor"&&editCaut!==null) {
    ...
    return (
      <div style={{...WRAP,paddingBottom:30}}>
        <div style={{background:"#7C1D1D",padding:"14px 16px",display:"flex",alignItems:"center",gap:10,position:"sticky",top:0,zIndex:10}}>
          <NavBack onClick={()=>{setCautDelCfm(false);setCautView(selCaut?"detail":"list");}}/>
          <span style={{color:"#fff",fontSize:17,fontWeight:800}}>{ec.id?"Edit Caution":"Flag Contact"}</span>
        </div>
        <div style={{padding:"12px 13px 20px",display:"flex",flexDirection:"column",gap:9}}>
          {/* Contact picker or display */}
          {!ec.id?(
            <InfoCard>
              <FL>Select Contact *</FL>
              <input value={cautPickSrch} onChange={e=>setCautPickSrch(e.target.value)} placeholder="Search My Network…" style={{...INP,border:`1px solid ${C.border}`,borderRadius:9,padding:"8px 10px",fontSize:13,marginBottom:8}}/>
              <div style={{maxHeight:220,overflowY:"auto",display:"flex",flexDirection:"column",gap:6}}>
                {availForCaut.map(c=>{const alreadyFlagged=cautions.some(k=>k.contactId===c.id&&k.id!==ec.id);return(
                  <div key={c.id} onClick={()=>!alreadyFlagged&&setEditCaut(p=>({...p,contactId:c.id}))}
                    style={{display:"flex",alignItems:"center",gap:10,padding:"9px 10px",borderRadius:10,cursor:alreadyFlagged?"default":"pointer",border:`2px solid ${ec.contactId===c.id?"#EF4444":C.border}`,background:ec.contactId===c.id?"#FEF2F2":alreadyFlagged?"#F9FAFB":C.white,opacity:alreadyFlagged?0.5:1}}>
                    <div style={{width:34,height:34,borderRadius:10,background:gradient(displayName(c)),display:"flex",alignItems:"center",justifyContent:"center",color:"#fff",fontSize:12,fontWeight:800,flexShrink:0}}>{initials(displayName(c))}</div>
                    <div style={{flex:1,minWidth:0}}>
                      <div style={{fontSize:13,fontWeight:700,color:C.text,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{displayName(c)}</div>
                      {(c.role||c.company)&&<div style={{fontSize:11,color:C.muted}}>{[c.role,c.company].filter(Boolean).join(" · ")}</div>}
                    </div>
                    {alreadyFlagged&&<span style={{fontSize:11,color:C.muted}}>Flagged</span>}
                    {ec.contactId===c.id&&<span style={{fontSize:14,color:"#EF4444"}}>✓</span>}
                  </div>
                );})}
              </div>
            </InfoCard>
            ...
            <InfoCard>
              <FL>Contact</FL>
              <div style={{display:"flex",alignItems:"center",gap:10}}>
                <div style={{width:40,height:40,borderRadius:12,background:gradient(displayName(selContact)),display:"flex",alignItems:"center",justifyContent:"center",color:"#fff",fontSize:14,fontWeight:800,flexShrink:0}}>{initials(displayName(selContact))}</div>
                <div><div style={{fontSize:15,fontWeight:700,color:C.text}}>{displayName(selContact)}</div>{(selContact.role||selContact.company)&&<div style={{fontSize:12,color:C.muted}}>{[selContact.role,selContact.company].filter(Boolean).join(" · ")}</div>}</div>
              </div>
            </InfoCard>
          ))}
          {/* Caution level */}
          <InfoCard>
            ...
            <div style={{display:"flex",gap:8}}>
              {CAUTION_LEVELS.map(l=>(
                <button key={l.id} onClick={()=>setEditCaut(p=>({...p,level:l.id}))}
                  style={{flex:1,padding:"10px 4px",borderRadius:10,border:`2px solid ${ec.level===l.id?l.color:C.border}`,background:ec.level===l.id?l.bg:"transparent",color:ec.level===l.id?l.color:C.muted,fontSize:12,fontWeight:700,cursor:"pointer",fontFamily:"inherit",textAlign:"center"}}>
                  {l.icon}<br/>{l.label}
                </button>
              ))}
            </div>
          ...
          <InfoCard>
            <FL>What They Did</FL>
            <textarea value={ec.whatTheyDid} onChange={e=>setEditCaut(p=>({...p,whatTheyDid:e.target.value}))} rows={4} placeholder="Describe what happened or what they did…" style={{...INP,resize:"none",lineHeight:1.55}}/>
          </InfoCard>
          {/* Why cautious */}
          <InfoCard>
            <FL>Why Be Cautious</FL>
            <textarea value={ec.whyCautious} onChange={e=>setEditCaut(p=>({...p,whyCautious:e.target.value}))} rows={4} placeholder="Why should you be careful around them? Any specific warnings…" style={{...INP,resize:"none",lineHeight:1.55}}/>
          </InfoCard>
          <button onClick={saveCaut} disabled={!okCaut}
            style={{background:okCaut?"#DC2626":C.border,border:"none",borderRadius:14,padding:15,color:okCaut?"#fff":C.muted,fontSize:15,fontWeight:800,cursor:okCaut?"pointer":"not-allowed",boxShadow:okCaut?"0 4px 14px rgba(220,38,38,0.35)":"none",fontFamily:"inherit"}}>
            {ec.id?"Save Changes":"Flag This Contact"}
          </button>
          {ec.id&&(!cautDelCfm?<button onClick={()=>setCautDelCfm(true)} style={{background:"transparent",border:`1.5px solid ${C.red}`,borderRadius:14,padding:13,color:C.red,fontSize:14,fontWeight:700,cursor:"pointer",fontFamily:"inherit"}}>Remove Flag</button>:(
            <InfoCard style={{border:`1px solid ${C.redBg}`}}>
              <div style={{fontSize:14,fontWeight:600,color:C.text,marginBottom:12,textAlign:"center"}}>Remove this caution flag?</div>
              <div style={{display:"flex",gap:9}}>
                <button onClick={()=>setCautDelCfm(false)} style={{flex:1,padding:11,border:`1px solid ${C.border}`,background:"transparent",borderRadius:11,fontSize:14,fontWeight:600,color:C.muted,cursor:"pointer",fontFamily:"inherit"}}>Cancel</button>
                <button onClick={deleteCaut} style={{flex:1,padding:11,border:"none",background:C.red,borderRadius:11,fontSize:14,fontWeight:700,color:"#fff",cursor:"pointer",fontFamily:"inherit"}}>Yes, Remove</button>
              </div>
            </InfoCard>
          ))}
        </div>
  ...
  if(cautView==="detail"&&selCaut) {
    const contact=contacts.find(c=>c.id===selCaut.contactId);
    const dn=contact?displayName(contact):"Deleted Contact";
    const level=cautLevelFor(selCaut.level);
    return (
      <div style={{...WRAP,paddingBottom:30}}>
        <div style={{background:"#7C1D1D",padding:"14px 16px 22px",position:"sticky",top:0,zIndex:10}}>
          <div style={{display:"flex",alignItems:"center",justifyContent:"space-between"}}>
            <NavBack onClick={()=>setCautView("list")}/>
            <button onClick={openCautEditor} style={{background:"rgba(255,255,255,0.15)",border:"none",color:"#fff",borderRadius:9,padding:"7px 14px",cursor:"pointer",fontSize:13,fontWeight:700,fontFamily:"inherit"}}>✏️ Edit</button>
          </div>
          ...
          <div style={{marginTop:10}}>
            <span style={{display:"inline-flex",alignItems:"center",gap:6,padding:"5px 12px",borderRadius:999,background:level.bg,color:level.color,fontSize:13,fontWeight:800}}>
        ...
        <div style={{padding:"12px 13px",display:"flex",flexDirection:"column",gap:9}}>
          ...
          {contact&&(
            <button onClick={()=>openDetail(contact,"caut")}
              style={{padding:13,background:C.navy,border:"none",borderRadius:13,fontSize:14,fontWeight:700,color:"#fff",cursor:"pointer",fontFamily:"inherit"}}>
              View Full Profile →
            </button>
          )}
        </div>
  ...
  return (
    <div style={{...WRAP,paddingBottom:TAB_H}}>
        ...
        <div>
          ...
          <div style={{display:"flex",gap:7,padding:"11px 13px 0",overflowX:"auto",scrollbarWidth:"none"}}>
            {["all",...industries].map(ind=>(
              <button key={ind} onClick={()=>setFilter(ind)} style={{flexShrink:0,padding:"6px 13px",borderRadius:999,fontSize:12,fontWeight:700,cursor:"pointer",border:"none",outline:"none",fontFamily:"inherit",background:filter===ind?C.navy:C.white,color:filter===ind?"#fff":C.muted,boxShadow:filter===ind?"0 2px 8px rgba(27,42,92,0.18)":"0 1px 2px rgba(0,0,0,0.07)"}}>
                {ind==="all"?"All":ind}
              </button>
            ))}
          </div>
          <div style={{padding:"8px 11px 0"}}>
            {sorted.length===0?(
              ...
              <div key={letter}>
                ...
                {grouped[letter].map(c=>{const ctx=ctxFor(c.context,contexts);const dn=displayName(c);const caut=cautions.find(k=>k.contactId===c.id);return(
                  <div key={c.id} onClick={()=>openDetail(c)} style={{background:C.white,borderRadius:13,marginBottom:6,padding:"13px",display:"flex",alignItems:"center",gap:11,boxShadow:"0 1px 3px rgba(0,0,0,0.06)",cursor:"pointer",borderLeft:caut?`4px solid ${cautLevelFor(caut.level).color}`:"4px solid transparent"}}>
          ...
          <div style={{padding:"12px 11px 0"}}>
            {trees.length===0?(
              <div style={{textAlign:"center",padding:"60px 24px",color:C.muted}}>
                ...
                <div style={{fontSize:13,lineHeight:1.7}}>Tap + to map out an organization, family, or social group.</div>
              </div>
            ):trees.map(tree=>{const typeInfo=treeTypeFor(tree.type);return(
              <div key={tree.id} onClick={()=>openTreeDetail(tree)} style={{background:C.white,borderRadius:14,marginBottom:10,padding:"14px 16px",cursor:"pointer",boxShadow:"0 1px 4px rgba(0,0,0,0.06)"}}>
                <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:5}}><div style={{fontSize:16,fontWeight:800,color:C.text}}>{tree.name}</div><MiniChip bg={typeInfo.bg} fg={typeInfo.fg}>{typeInfo.icon} {typeInfo.label}</MiniChip></div>
                <div style={{fontSize:12,color:C.muted,marginBottom:10}}>{tree.members.length} {tree.members.length===1?"person":"people"}</div>
                <div style={{display:"flex",alignItems:"center",justifyContent:"space-between"}}>
                  <div style={{display:"flex"}}>{tree.members.slice(0,6).map((m,i)=>{const contact=contacts.find(c=>c.id===m.contactId);const dn=contact?displayName(contact):"?";return(<div key={m.id} style={{width:30,height:30,borderRadius:9,background:gradient(dn),display:"flex",alignItems:"center",justifyContent:"center",color:"#fff",fontSize:10,fontWeight:800,border:"2px solid #fff",marginLeft:i>0?-8:0,zIndex:tree.members.length-i}}>{initials(dn)}</div>);})}{tree.members.length>6&&<div style={{width:30,height:30,borderRadius:9,background:C.muted,display:"flex",alignItems:"center",justifyContent:"center",color:"#fff",fontSize:10,fontWeight:800,border:"2px solid #fff",marginLeft:-8}}>+{tree.members.length-6}</div>}</div>
      ...
      {activeTab==="cautious"&&(
        <div>
          <div style={{background:"#7C1D1D",padding:"14px 16px 16px",position:"sticky",top:0,zIndex:10}}>
            <div style={{color:"#fff",fontSize:21,fontWeight:800,letterSpacing:-0.5}}>⚠️ Cautious</div>
            <div style={{color:"rgba(255,255,255,0.55)",fontSize:11,marginTop:1}}>{cautions.length} {cautions.length===1?"flagged contact":"flagged contacts"}</div>
          </div>
          <div style={{padding:"12px 11px 0"}}>
            {cautions.length===0?(
              <div style={{textAlign:"center",padding:"60px 24px",color:C.muted}}>
                <div style={{fontSize:52,marginBottom:14}}>⚠️</div>
                <div style={{fontSize:17,fontWeight:700,color:"#374151",marginBottom:8}}>No flagged contacts</div>
                <div style={{fontSize:13,lineHeight:1.7}}>Tap + to flag someone from your network you need to be careful with.</div>
              </div>
            ):[...cautions].sort((a,b)=>{const order={red:0,orange:1,yellow:2};return(order[a.level]??3)-(order[b.level]??3);}).map(caut=>{
              const contact=contacts.find(c=>c.id===caut.contactId);
              const dn=contact?displayName(contact):"Deleted Contact";
              const level=cautLevelFor(caut.level);
              return(
                <div key={caut.id} onClick={()=>openCautDetail(caut)}
                  style={{background:C.white,borderRadius:13,marginBottom:8,padding:"14px",display:"flex",alignItems:"center",gap:12,boxShadow:"0 1px 3px rgba(0,0,0,0.06)",cursor:"pointer",borderLeft:`4px solid ${level.color}`}}>
                  <div style={{width:46,height:46,borderRadius:13,background:contact?gradient(dn):"#6B7280",display:"flex",alignItems:"center",justifyContent:"center",color:"#fff",fontSize:15,fontWeight:800,flexShrink:0}}>{initials(dn)}</div>
                  <div style={{flex:1,minWidth:0}}>
                    <div style={{display:"flex",alignItems:"center",gap:7,marginBottom:3}}>
                      <div style={{fontSize:15,fontWeight:700,color:C.text,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{dn}</div>
                      <span style={{flexShrink:0,fontSize:11,fontWeight:700,padding:"2px 7px",borderRadius:999,background:level.bg,color:level.color}}>{level.label}</span>
                    </div>
                    {(contact?.role||contact?.company)&&<div style={{fontSize:11,color:C.muted,marginBottom:4}}>{[contact.role,contact.company].filter(Boolean).join(" · ")}</div>}
                    {caut.whatTheyDid&&<div style={{fontSize:12,color:C.muted,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{caut.whatTheyDid}</div>}
                  </div>
                  <span style={{color:"#D1D5DB",fontSize:18,flexShrink:0}}>›</span>
                </div>
              );
